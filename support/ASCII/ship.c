...
#define SHIPVER "ship version 1.7  July 18 1994 Mark Adler"
...
#if defined(MSDOS) && !defined(ATARI_ST) /* Use binary mode for binary files */
#  include <io.h>
#  include <fcntl.h>
#endif
    ...
    */
    ...
    case 4:
      ...
      else if (strncmp(l, "cont ", 5) == 0)
      {
        ...
        for (q = l + 5; *q && *q != ' '; q++)
          ;
        if (*q == 0 || (ulg) atol(l + 5) != ccnt + 4 + (decn != 0) ||
            strcmp(q + 1, r))
          err(SE_CONT, r);
        b = 0;
      }
